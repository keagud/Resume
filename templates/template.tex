...
\begin{document}
    ...
    {% set exp_role = loc.experience_role %}
        ...
        {% if end.year == start.year %}
          {% set time_display  = "%s - %s"|format(start.strftime("%B %Y"), end.strftime("%B %Y" )) %}
        {% else %}
          {% set time_display = "%d - %d"|format(start.year, end.year) %}
        {% endif %}


        \begingroup
      \textbf{ {{ loc.experience_location_name }} }\hfill\color{black!70}\small{ {{time_display}} }
        \endgroup
        \textit{ {{exp_role.role_name}} }

        {% if exp_role.role_description or loc.action_bullet %}
      \begin{itemize}

        {% if exp_role.role_description %}
        \item {{ exp_role.role_description }}
            {% endif %}
